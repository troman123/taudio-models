...
class LightningSystem(pl.LightningModule):
    ...
    def predict_step(
        ...
        basestem_names = list(output["audio"].keys())
        if get_residual:
        ...
        if get_no_muc_combinations:
            no_muc_stems = [
                    stem for stem in output["audio"] if
                    stem not in self._MUSC_STEMS
            ]
            no_muc_combinations = chain.from_iterable(
                    combinations(no_muc_stems, r) for r in
                    range(2, len(no_muc_stems) + 1)
            )
            for combination in no_muc_combinations:
                combination_ = list(combination)
                output["audio"]["+".join(combination_)] = sum(
                        [output["audio"][stem] for stem in combination_]
                )
        ...
        for b in range(batch_size):
            for stem in output["audio"]:
                ...
                if out_isfile:
                    ...
                    outresults[stem] = outfile
                else:
                    outresults[stem] = output["audio"][stem][b, ...].cpu().detach() #.numpy().copy()

        return outresults, basestem_names
